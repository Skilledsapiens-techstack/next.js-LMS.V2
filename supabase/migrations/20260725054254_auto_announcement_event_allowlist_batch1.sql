alter table public.portal_update_events
  drop constraint if exists portal_update_events_event_type_check;

alter table public.portal_update_events
  add constraint portal_update_events_event_type_check check (
    event_type in (
...
declare
  ...
  portal_update_date date := ((now() at time zone 'Asia/Kolkata')::date);
...
begin
  ...
  elsif clean_event_type in ('resource_added', 'career_readiness_added') and not (is_service_role or public.admin_has_permission('admin.resources.manage')) then
  ...
  elsif clean_event_type in ('session_cancelled', 'session_completed', 'session_rescheduled', 'session_scheduled') and not (is_service_role or public.admin_has_permission('admin.meetings.manage')) then
    raise exception 'Not authorized to create session portal updates.' using errcode = '42501';
  elsif clean_event_type in ('support_ticket_answered', 'support_ticket_resolved') and not (is_service_role or public.admin_has_permission('admin.support.manage')) then
    raise exception 'Not authorized to create support portal updates.' using errcode = '42501';
  end if;
  ...
  loop
    ...
    if found then
      ...
      digest_source_key := 'daily-portal-updates:' || portal_update_date::text || ':' || target.email;

      select count(*)
      into event_total
      from public.portal_update_events e
      where e.student_email = target.email
        and e.event_date = portal_update_date;
      ...
      values (
        'AUTO-PORTAL-' || to_char(portal_update_date, 'YYYYMMDD') || '-' || substr(md5(target.email), 1, 10),
        'student',
        array[target.email],
        ...
        false,
        portal_update_date,
        portal_update_date + 1,
        now() + interval '24 hours',
        'Open dashboard',
        '/student',
        'daily_portal_updates',
        portal_update_date::text,
        digest_source_key,
        true,
        jsonb_build_object('eventCount', event_total, 'eventDate', portal_update_date, 'studentEmail', target.email),
        nullif(trim(coalesce(p_created_by, '')), ''),
        nullif(trim(coalesce(p_created_by, '')), '')
      )
      ...
      where student_email = target.email
        and event_date = portal_update_date
