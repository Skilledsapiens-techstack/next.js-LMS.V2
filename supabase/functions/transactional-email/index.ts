...
function passwordEmailIntentFromRedirect(value: unknown, fallback: 'create' | 'forgot' = 'forgot') {
  const redirectUrl = text(value);
  try {
    const url = new URL(redirectUrl);
    return url.searchParams.get('intent') === 'create' ? 'create' : fallback;
  } catch (_err) {
    return fallback;
  }
}
