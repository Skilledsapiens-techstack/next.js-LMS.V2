...
import { CertificatePdfRenderInput, CertificatePdfRenderPlan, createCertificatePdfRenderPlan } from './certificate-pdf-render-plan';

type CertificatePdfRenderJobRow = {
...
export type CertificatePdfRenderLoadInput = CertificatePdfRenderInput & {
  jobIdempotencyKey: string;
};

export type CertificatePdfRenderLoadResult = {
  status: 'ready' | 'not_found';
  plan?: CertificatePdfRenderPlan;
  message: string;
};
export class CertificatePdfRenderLoader {
  constructor(private readonly supabase: SupabaseService) {}

  async loadPlan(input: CertificatePdfRenderLoadInput): Promise<CertificatePdfRenderLoadResult> {
    ...
    return {
      status: 'ready',
      plan: createCertificatePdfRenderPlan(this.toCertificateGenerationJob(job), {
        workerId,
  ...
  private async loadGenerationJob(idempotencyKey: string): Promise<CertificatePdfRenderJobRow | undefined> {
    ...
    if (error) {
      throw new ServiceUnavailableException(`Unable to load certificate generation job for PDF rendering: ${error.message}`);
   }

    return this.asCertificatePdfRenderJobRow(data);
 }

  private toCertificateGenerationJob(row: CertificatePdfRenderJobRow): CertificateGenerationJob {
    return {
  ...
  private asCertificatePdfRenderJobRow(value: unknown): CertificatePdfRenderJobRow | undefined {
    if (
